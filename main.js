function criarCracha() {
  let VisitorsName = document.getElementById('txtn')
  ...
  if (VisitorsName.value.length < 1) {
    VisitorsName.style.backgroundColor = '#ff00003f'
    alert(
      'Obrigatório preencher seu nome completo! Senão como vou gerar o seu crachazinho bb?'
    )
    VisitorsName.focus()
    return false
  } else if (VisitorsGithub.value.length < 1) {
    ...
    localStorage.setItem('VisitorsName', JSON.stringify(VisitorsName.value))
...
let VisitorsName = JSON.parse(localStorage.getItem('VisitorsName'))
